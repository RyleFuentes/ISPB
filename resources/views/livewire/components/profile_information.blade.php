    ...
    <div class="col-md-5 d-flex align-items-center justify-content-center">
        <div class="bg-primary rounded py-3 px-3 bg-opacity-25 w-100 h-100">
            <div class="d-flex align-items-center justify-content-center py-1">
                <div class="prof rounded bg-primary bg-opacity-25 border border-3 border-light">
                    <img class="img prof" src="{{ asset('images/logo.png') }}" alt="logo">
                </div>
            </div>
            <hr>
            <div>
                <p class="fs-4 text-center">{{ $user->name }}</p>
                @if ($user->role == 0)
                    <p class="fs-6 text-center">Admin</p>
                @else
                    <p class="fs-6 text-center">Inventory Clerk</p>
                @endif
            </div>
        </div>
    </div>
